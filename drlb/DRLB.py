...
def state_(init_budget,
    ...
    # slot_state = [current_day_budget[slot] / init_budget, ROL_t, BCR_t, CPM_t, WR_t, slot_origin_reward]
    # slot_state = [current_day_budget[slot] / init_budget, BCR_t, CPM_t, WR_t, slot_origin_reward]
    # slot_state = [current_day_budget[slot] / init_budget, BCR_t, CPM_t, WR_t]
    # slot_state = [BCR_t, CPM_t, WR_t]
    # slot_state = [BCR_t, CPM_t]
    slot_state = [ROL_t]
...
def choose_init_lamda(budget_para):
    data = pd.read_csv(config['data_path'] + 'base_bid.csv')
    dataset_dict = {
        '1458': 0,
        '3358': 1,
        '3427': 2,
    ...
    dataset = dataset_dict[config['campaign_id'].strip('/')]
    avg_pctr = data.loc[dataset, 'avg_pctr']
    base_bid = data.loc[dataset, str(budget_para)]
...
def run_train(budget_para):
    ...
    if config['time_fraction'] == 96:
        ...
        train_data = train_data.iloc[:, [0, 1, 2, 4, 6]]
    elif config['time_fraction'] == 24:
        train_data = train_data.iloc[:, [0, 1, 2, 4, 5]]
    ...
    init_lambda = choose_init_lamda(budget_para)
    ...
    for epoch in range(config['train_episodes']):
    ...
    record_path = 'result-96-1-rol/' + config['campaign_id']
...
def run_test(budget_para):
    ...
    if config['time_fraction'] == 96:
        ...
        test_data = test_data.iloc[:, [0, 1, 2, 4, 6]]
    elif config['time_fraction'] == 24:
        test_data = test_data.iloc[:, [0, 1, 2, 4, 5]]
...
if __name__ == '__main__':
    config = {
        'data_path': '../data/ipinyou/',
        'campaign_id': '1458/',
        'time_fraction': 96,
        'e_greedy': 0.9,  # epsilon
        'learning_rate': 1e-3,  # 学习率
        'pg_learning_rate': 1e-3,  # 学习率
        'reward_decay': 1.0,  # 折扣因子
        'feature_num': 1,  # 总特征维度
        'action_numbers': 7,
        'budget_para': [2, 4, 8, 16],
        'train_episodes': 1000,
        'neuron_nums': 100,
    ...
    for i in range(len(budget_para_list)):
        ...
        Rewardnet = RewardNet([-0.08, -0.03, -0.01, 0, 0.01, 0.03, 0.08],  # 按照数据集中的“块”计量
                              action_numbers=1, reward_numbers=1, feature_numbers=config['feature_num'],
