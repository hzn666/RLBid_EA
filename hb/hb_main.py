        ...
        try:
            with tqdm(range(day_real_imps)) as tqdm_t:
                for impression_index in tqdm_t:
                    ...
                    if bid_price >= market_prices[impression_index] and day_spend + market_prices[impression_index] <= today_budget:
...
def bid_slot(average_pctr, data, total_budget_list, budget_prop, base_bid, test_flag=False):
    # 数据统计
    real_imps = len(data)
    budget = np.divide(total_budget_list, budget_prop)
    ...
    for day_index, day in enumerate(data['96_time_fraction'].unique()):
        current_day_data = data[data['96_time_fraction'].isin([day])]
        clks = list(current_day_data['clk'])
        pctrs = list(current_day_data['pctr'])
        market_prices = list(current_day_data['market_price'])
        ...
        try:
            with tqdm(range(day_real_imps)) as tqdm_t:
                for impression_index in tqdm_t:
                    day_real_clks += clks[impression_index]
                    bid_price = get_lin_bid(average_pctr, pctrs[impression_index], base_bid)
                    if bid_price >= market_prices[impression_index] and day_spend + pctrs[impression_index] <= today_budget:
                        day_win_pctr += pctrs[impression_index]
                        day_win_imps += 1
                        day_win_clks += clks[impression_index]
                        day_spend += market_prices[impression_index]
                        ...
                        if test_flag:
                            bid_action.append([pctrs[impression_index], bid_price, market_prices[impression_index],
                                               clks[impression_index], minutes[impression_index], 0])
                    if not early_stop:
                        if day_spend + bid_price > today_budget:
                            early_stop = time_frac[impression_index]
        except KeyboardInterrupt:
            tqdm_t.close()
            raise
        ...
        win_imps += day_win_imps
        win_pctr += day_win_pctr
        day_cpm = day_spend / day_real_imps
        day_cpm_list.append(day_cpm)
        spend.append(day_spend)
...
def train(cfg, test_flag=False):
    ...
    total_train_log.to_csv(cfg.train_log_path + '{}_total_train_bid_log.csv'.format(cfg.train_type), index=False)
    ...
    day_train_log.to_csv(cfg.train_log_path + '{}_day_train_bid_log.csv'.format(cfg.train_type), index=False)

    print('存储全体训练最优出价')
    best_base_bid = total_train_log.groupby(['budget_prop']).apply(lambda x: x[x.win_clks == x.win_clks.max()])
    best_base_bid.to_csv(cfg.train_log_path + '{}_total_train_best_base_bid.csv'.format(cfg.train_type), index=False)
    ...
    for day in train_data.day.unique():
        ...
        day_best_base_bid.to_csv(cfg.train_log_path +
                                 '{}_day{}_train_best_base_bid.csv'.format(cfg.train_type, day), index=False)

    if test_flag:
        test(cfg)


def train_slot(cfg, test_flag=False):
    print('训练类型：', cfg.train_type)
    ...
    for budget_prop in cfg.budget_prop_list:
        for base_bid in base_bid_list:
            print([camp, budget_prop, base_bid])
            train_result_temp, day_train_result_temp = bid_slot(average_pctr, train_data, total_budget_list, budget_prop,
                                                           base_bid)
            train_result.append(train_result_temp)
            day_train_result.extend(day_train_result_temp)
    ...
    print('存储全体训练最优出价')
    best_base_bid = total_train_log.groupby(['budget_prop']).apply(lambda x: x[x.win_pctr == x.win_pctr.max()])
    best_base_bid.to_csv(cfg.train_log_path + '{}_total_train_best_base_bid.csv'.format(cfg.train_type), index=False)
    ...
    for day in train_data.day.unique():
        current_day_data = day_train_log[day_train_log.day.isin([day])]
        day_best_base_bid = current_day_data.groupby(['budget_prop']).apply(
            lambda x: x[x.day_win_pctr == x.day_win_pctr.max()])
        day_best_base_bid.to_csv(cfg.train_log_path +
                                 '{}_day{}_train_best_base_bid.csv'.format(cfg.train_type, day), index=False)
...
def test(cfg):
    ...
    for budget_prop in cfg.budget_prop_list:
        best_base_bid_df = pd.read_csv(cfg.train_log_path + '{}_total_train_best_base_bid.csv'.format(cfg.train_type))
        ...
        bid_action_df.to_csv(cfg.test_log_path + '{}_{}_test_bid_action.csv'.format(cfg.train_type, budget_prop),
                             index=False)
    ...
    pd.DataFrame(data=test_result,
        ...
        cfg.test_log_path + '{}_total_test_bid_log.csv'.format(cfg.train_type), index=False
    )
    ...
    pd.DataFrame(data=day_test_result,
        ...
        cfg.test_log_path + '{}_day_test_bid_log.csv'.format(cfg.train_type), index=False
    )
